...
void test_ac_exp_cordic_ifloat(
...
)
{
  ...
  for (double i = lower_limit; i <= upper_limit; i += step) {
    ...
    input = i;
    test_ac_exp_cordic_fixed(input, output_fixed);
...
//   range of valid values on that type in order to compare the precision of the
//   cordic exp model with the computed power using a standard C double type.
  ...
  for (int i = 0; i < exp_arr_size; i++) {
    ...
    for (ac_int<Wfl - 1, false> mant_i = lower_limit_it; mant_i <= upper_limit_it; mant_i += step_it) {
      ...
      if (input_float < input_max) {
        test_ac_exp_cordic_float(input_float, output_float);

        expected_value_float = ((T_out)exp(input_float.to_double())).to_double();
      ...
      test_ac_exp_cordic_float(input_float_neg, output_float);
...
//   precision of the cordic exp model with the computed exp using
...
int test_driver_stfloat(
  ...
  for (int i = 0; i < exp_arr_size_stfloat; i++) {
    for (ac_int<W2acfl - 1, false> mant_i = lower_limit_stfloat; mant_i <= upper_limit_stfloat; mant_i += step_stfloat) {
      ...
      if (input_stfloat < input_max) {
        test_ac_exp_cordic_stfloat<OR_TF>(input_stfloat, output_stfloat);
      
        expected_value_stfloat = ((T_out)exp(input_acfl_norm.to_double())).to_double();
      ...
      test_ac_exp_cordic_stfloat<OR_TF>(input_stfloat_neg, output_stfloat);
  ...
  T_out output_nan;
  test_ac_exp_cordic_stfloat<OR_TF>(input_nan, output_nan);
...
int test_driver_ifloat(
  ...
  for (int i = 0; i < exp_arr_size_ifloat; i++) {
    for (ac_int<W2acfl - 1, false> mant_i = lower_limit_ifloat; mant_i <= upper_limit_ifloat; mant_i += step_ifloat) {
      ...
      if (input_ifloat < input_max) {
        T_out output_ifloat;
        test_ac_exp_cordic_ifloat<OR_TF>(input_ifloat, output_ifloat);
      
        expected_value_ifloat = ((T_out)exp(input_acfl_norm.to_double())).to_ac_float().to_double();
      ...
      test_ac_exp_cordic_ifloat<OR_TF>(input_ifloat_neg, output_ifloat);
      
      expected_value_ifloat = ((T_out)exp(input_acfl_norm_neg.to_double())).to_ac_float().to_double();
      actual_value_ifloat   = output_ifloat.to_ac_float().to_double();
  ...
  T_in input_nan = T_in::nan();
  T_out output_nan;
  test_ac_exp_cordic_ifloat<OR_TF>(input_nan, output_nan);
  bool nan_testing_passed = output_nan.isnan() && !output_nan.signbit();
