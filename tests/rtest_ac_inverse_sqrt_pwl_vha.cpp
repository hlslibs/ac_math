...
// ac_inverse_sqrt_pwl_vha() function using a variety of data types and bit-
// widths.

// To compile standalone and run:
//   $MGC_HOME/bin/c++ -std=c++11 -I$MGC_HOME/shared/include rtest_ac_inverse_sqrt_pwl_vha.cpp -o design
//   ./design
...
// ==============================================================================
// Test Designs
//   These simple functions allow executing the ac_inverse_sqrt_pwl_vha() function
...
void test_ac_inverse_sqrt_pwl_vha_fixed(
...
{
  out1 = ac_inverse_sqrt_pwl_vha<ac_fixed<outWfi, outIfi, false, AC_TRN, AC_WRAP> >(in1);
  out2 = ac_inverse_sqrt_pwl_vha<ac_complex<ac_fixed<outWfi + 1, outIfi + 1, true, AC_TRN, AC_WRAP> > >(in2);
}

// Test Design for real fixed point values.
template <int Wfi, int Ifi, int outWfi, int outIfi>
void test_ac_inverse_sqrt_pwl_vha_real_fixed(
  const ac_fixed<Wfi, Ifi, false, AC_TRN, AC_WRAP> &in1,
  ac_fixed<outWfi, outIfi, false, AC_TRN, AC_WRAP> &out1
)
{
...
void test_ac_inverse_sqrt_pwl_vha_float(
  const ac_float<Wfl, Ifl, Efl, AC_TRN>   &in1,
  ac_float<outWfl, outIfl, outEfl, AC_TRN>   &out1
)
{
  out1 = ac_inverse_sqrt_pwl_vha<ac_float<outWfl, outIfl, outEfl, AC_TRN> >(in1);
}
...
)
{
  ...
  cout << "TEST: ac_inverse_sqrt_pwl_vha() INPUTS: ";
  cout.width(38);
  ...
  for (double i = lower_limit_complex; i <= upper_limit_complex; i += step_fixed) {
    ...
    for (double j = lower_limit_complex; j <= upper_limit_complex; j += step_fixed) {
      ...
      test_ac_inverse_sqrt_pwl_vha_fixed(input_fixed, output_fixed, cmplx_input_fixed, cmplx_output_fixed);
...
)
{
  ...
  for (double i = lower_limit_fixed; i <= upper_limit_fixed; i += step_fixed) {
    ...
    test_ac_inverse_sqrt_pwl_vha_real_fixed(input_fixed, output_fixed);
...
)
{
  ...
  cout << "TEST: ac_inverse_sqrt_pwl_vha() INPUT:  ";
  cout.width(38);
  ...
  for (int i = 0; i < exp_arr_size; i++) {
    ...
    for (ac_int<Wfl - 1, false> mant_i = lower_limit_it; mant_i <= upper_limit_it; mant_i += step_it) {
      ...
      T_out output_float;
      test_ac_inverse_sqrt_pwl_vha_float(input_float, output_float);
...
int main(int argc, char *argv[])
{
  ...
  // Set tolerance
  double allowed_error_fixed = 0.005;
  ...
  double threshold_fixed = 0.00005;

  // Set tolerance
  double allowed_error_float = 0.005;
  // threshold below which we calculate absolute error instead of relative for floating point
  double threshold_float = 0.00005;

  cout << "=============================================================================" << endl;
  cout << "Testing function: ac_inverse_sqrt_pwl_vha() - Allowed error " << allowed_error_fixed << " (fixed pt), " << allowed_error_complex << " (complex fixed pt), " << allowed_error_float << " (float pt)" << endl;
  ...
  test_driver_float< 19,  1, 8, 21,  2,  8>(max_error_float, allowed_error_float, threshold_float);
  test_driver_float< 19,  2, 8, 21,  2,  8>(max_error_float, allowed_error_float, threshold_float);
  test_driver_float< 19,  3, 8, 21,  2,  8>(max_error_float, allowed_error_float, threshold_float);
  test_driver_float< 21, -1, 8, 21, -1,  8>(max_error_float, allowed_error_float, threshold_float);
  test_driver_float< 21, -2, 8, 21, -2,  8>(max_error_float, allowed_error_float, threshold_float);
  test_driver_float< 21, -3, 8, 21, -3,  8>(max_error_float, allowed_error_float, threshold_float);
  test_driver_float< 21, -3, 8, 21, -3,  8>(max_error_float, allowed_error_float, threshold_float);
  ...
  test_driver_float< 19, -2, 8, 21, 2,  8>(max_error_float, allowed_error_float, threshold_float);
  test_driver_float< 19, -3, 8, 21, 2,  8>(max_error_float, allowed_error_float, threshold_float);
  test_driver_float< 19, -7, 8, 21, 2,  8>(max_error_float, allowed_error_float, threshold_float);
  test_driver_float< 19, -8, 8, 21, 2,  8>(max_error_float, allowed_error_float, threshold_float);
  ...
  if (test_fail) {
    cout << "  ac_inverse_sqrt_pwl_vha - FAILED - Error tolerance(s) exceeded" << endl; // LCOV_EXCL_LINE
    cout << "=============================================================================" << endl; // LCOV_EXCL_LINE
    return -1; // LCOV_EXCL_LINE
  } else {
    cout << "  ac_inverse_sqrt_pwl_vha - PASSED" << endl;
    cout << "=============================================================================" << endl;
  }
  return 0;
}
