...
void test_ac_log_cordic_ifloat(
...
)
{
  ...
  for (i = lower_limit; i <= upper_limit; i += step) {
    ...
    test_ac_log_cordic_fixed(input,output_fixed);

    double actual_log = output_fixed.to_double();
    double this_error_log = abs_double(expected_log - actual_log) * 100.0;
    ...
    if (this_error_log > max_error_fixed) { max_error_fixed = this_error_log; }
  }
...
)
{
  ...
  for (int i = 0; i < exp_arr_size; i++) {
    ...
    for (ac_int<Wfl - 1, false> mant_i = lower_limit_it; mant_i <= upper_limit_it; mant_i += step_it) {
      ...
      T_out output_float;
      test_ac_log_cordic_float(input_float, output_float);
...
//   precision of the cordic log model with the computed log using
...
int test_driver_stfloat(
  ...
  for (int i = 0; i < exp_arr_size_stfloat; i++) {
    for (ac_int<W2acfl - 1, false> mant_i = lower_limit_stfloat; mant_i <= upper_limit_stfloat; mant_i += step_stfloat) {
      ...
      test_ac_log_cordic_stfloat<OR_TF>(input_stfloat, output_stfloat);
      
      double expected_value_stfloat = ((T_out)log(input_acfl_norm.to_double())).to_double();
  ...
  T_out output_nan;
  test_ac_log_cordic_stfloat<OR_TF>(input_nan, output_nan);
...
int test_driver_ifloat(
  ...
  for (int i = 0; i < exp_arr_size_ifloat; i++) {
    for (ac_int<W2acfl - 1, false> mant_i = lower_limit_ifloat; mant_i <= upper_limit_ifloat; mant_i += step_ifloat) {
      ...
      test_ac_log_cordic_ifloat<OR_TF>(input_ifloat, output_ifloat);
      
      double expected_value_ifloat = ((T_out)log(input_acfl_norm.to_double())).to_ac_float().to_double();
  ...
  T_in input_nan = T_in::nan();
  T_out output_nan;
  test_ac_log_cordic_ifloat<OR_TF>(input_nan, output_nan);
  bool nan_testing_passed = output_nan.isnan() && !output_nan.signbit();
