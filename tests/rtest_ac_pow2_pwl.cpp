...
// ==============================================================================
// Test Design
//   These simple functions allow executing the ac_pow2_pwl() function
...
)
{
  ...
  for (double i = lower_limit; i <= upper_limit; i += step) {
    ...
    input_fixed = i;
    test_ac_pow2_pwl_fixed(input_fixed, output_pow2);

    double expected_value_pow2 = ((T_out)pow(2, input_fixed.to_double())).to_double();
    ...
    if (expected_value_pow2 > threshold_fixed) {
    ...
    if (check_monotonic) {
      ...
      if (compare_pow2) {
        ...
        if (old_output_pow2 > actual_value_pow2) {
          ...
          cout << "x = " << input_fixed << endl; // LCOV_EXCL_LINE
    ...
    if (this_error_pow2 > max_error_fixed) {max_error_fixed = this_error_pow2;}
  }
  ...
  for (int i = 0; i < exp_arr_size; i++) {
    ...
    for (ac_int<Wfl - 1, false> mant_i = lower_limit_it; mant_i <= upper_limit_it; mant_i += step_it) {
      ...
      if (input_float >= input_max) {
        // Break if pow2(input) is greater than the max value supported by the output.
        break;
      }
      T_out output_float;
      test_ac_pow2_pwl_float(input_float, output_float);

      double expected_value_float = ((T_out)pow(2.0, input_float.to_double())).to_double();
      ...
      test_ac_pow2_pwl_float(input_float_neg, output_float);

      expected_value_float = ((T_out)pow(2.0, input_float_neg.to_double())).to_double();
...
int test_driver_stfloat(
  ...
  for (int i = 0; i < exp_arr_size_stfloat; i++) {
    for (ac_int<W2acfl - 1, false> mant_i = lower_limit_stfloat; mant_i <= upper_limit_stfloat; mant_i += step_stfloat) {
      ...
      if (input_stfloat >= input_max) {
        // Break if pow2(input) is greater than the max/lesser than the min value supported by the output.
        break;
      }
      T_out output_stfloat;
      test_ac_pow2_pwl_stfloat(input_stfloat, output_stfloat);
      
      double expected_value_stfloat = ((T_out)pow(2.0, input_acfl_norm.to_double())).to_double();
      ...
      test_ac_pow2_pwl_stfloat(input_stfloat_neg, output_stfloat);
      
      expected_value_stfloat = ((T_out)pow(2.0, input_acfl_norm_neg.to_double())).to_double();
  ...
  T_out output_nan;
  test_ac_pow2_pwl_stfloat(input_nan, output_nan);
...
int test_driver_ifloat(
  ...
  for (int i = 0; i < exp_arr_size_ifloat; i++) {
    for (ac_int<W2acfl - 1, false> mant_i = lower_limit_ifloat; mant_i <= upper_limit_ifloat; mant_i += step_ifloat) {
      ...
      T_out output_ifloat;
      test_ac_pow2_pwl_ifloat(input_ifloat, output_ifloat);
      
      double expected_value_ifloat = pow(2.0, input_acfl_norm.to_double());
      ...
      test_ac_pow2_pwl_ifloat(input_ifloat_neg, output_ifloat);
      
      expected_value_ifloat = pow(2.0, input_acfl_norm_neg.to_double());
      expected_value_ifloat = (T_out(expected_value_ifloat)).to_ac_float().to_double();
  ...
  T_in input_nan = T_in::nan();
  T_out output_nan;
  test_ac_pow2_pwl_ifloat(input_nan, output_nan);
  bool nan_testing_passed = output_nan.isnan() && !output_nan.signbit();
...
int main(int argc, char *argv[])
{
  ...
  // template <int Wfi, int Ifi, bool Sfi, int outWfi, int outIfi>
  test_driver_fixed<  8,  3,  true, 64, 32>(max_error_fixed, allowed_error, threshold);
  ...
  test_driver_fixed< 18,  5,  true, 64, 32>(max_error_fixed, allowed_error, threshold);
  ...
  test_driver_fixed<  2,  5, false, 64, 32>(max_error_fixed, allowed_error, threshold);
  test_driver_fixed< 18,  4, false, 64, 32>(max_error_fixed, allowed_error, threshold);
