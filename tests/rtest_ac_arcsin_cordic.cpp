...
void test_ac_arcsin_cordic_fixed(
...
void test_ac_arcsin_cordic_float(
...
)
{
  ...
  for (double i = -1; i < 1; i += step) {
    ...
    input = i;
    test_ac_arcsin_cordic_fixed(input, out_sin);
  ...
  passed = (max_error_arcsine < allowed_error_fixed);
...
int test_driver_float(
  double &cummulative_max_error_arcsine_float,
...
)
{
  ...
  for (int i = 0; i < exp_arr_size; i++) {
    ...
    // variable does not overflow.
      ...
      for (ac_int<Wfl - 1, false> mant_i = lower_limit_it; mant_i <= upper_limit_it; mant_i += step_it) {
        ...
        T_out arcsin; 
        double expected_arcsine_value, actual_arcsine_value;
        if (input_float <= 1) 
        { 
          test_ac_arcsin_cordic_float(input_float, arcsin);
          expected_arcsine_value   = ((T_out)(asin(input_float.to_double())/M_PI)).to_double();
          actual_arcsine_value     = arcsin.to_double();
        ...
        {
          test_ac_arcsin_cordic_float(input_float_neg, arcsin);
          expected_arcsine_value   = ((T_out)(asin(input_float_neg.to_double())/M_PI)).to_double();
          actual_arcsine_value     = arcsin.to_double();
  ...
  #ifdef DEBUG
        if (this_error_arcsine > allowed_error_float) {
          cout << endl;
          cout << "  Error exceeds tolerance" << endl;
          cout << "  input_float      = " << input_float << endl;
          cout << "  expected_arcsine_value   = " << expected_arcsine_value << endl;
          cout << "  actual_arcsine_value     = " << actual_arcsine_value << endl;
          cout << "  this_error_arcsine = " << this_error_arcsine << endl;
        }
  #endif
      }
  }

      passed = (max_error_arcsine < allowed_error_float);

      if (passed) { printf("PASSED , max err (%f sin) \n", max_error_arcsine); }
      else        { printf("FAILED , max err (%f sin) \n", max_error_arcsine); } // LCOV_EXCL_LINE

      if (max_error_arcsine>cummulative_max_error_arcsine_float) { cummulative_max_error_arcsine_float = max_error_arcsine; }
...
{
  double max_error_arcsine_fixed = 0, max_error_arcsine_float = 0;
  ...
  test_driver_fixed< 12,  1,  true, 24, 2, true>(max_error_arcsine_fixed, allowed_error);
  test_driver_fixed<  2,  0,  true, 27, 2, true>(max_error_arcsine_fixed, allowed_error);
  test_driver_fixed< 12,  0,  true, 24, 3, true>(max_error_arcsine_fixed, allowed_error);
  test_driver_fixed<  4,  1,  true, 25, 3, true>(max_error_arcsine_fixed, allowed_error);
  test_driver_fixed<  5, -2,  true, 30, 4, true>(max_error_arcsine_fixed, allowed_error);
  test_driver_fixed<  3, -2,  true, 28, 2, true>(max_error_arcsine_fixed, allowed_error);
  test_driver_fixed<  2, -2,  true, 32, 2, true>(max_error_arcsine_fixed, allowed_error);
  test_driver_fixed< 11,  1,  true, 34, 2, true>(max_error_arcsine_fixed, allowed_error);
  test_driver_fixed<  8, -3,  true, 25, 2, true>(max_error_arcsine_fixed, allowed_error);
  test_driver_fixed<  9, -3,  true, 26, 2, true>(max_error_arcsine_fixed, allowed_error);
  test_driver_fixed< 14,  2,  true, 24, 2, true>(max_error_arcsine_fixed, allowed_error);
  test_driver_fixed< 12,  2,  true, 24, 2, true>(max_error_arcsine_fixed, allowed_error);

  // template <int Wfl, int Ifl, int Efl, int outWfl, int outIfl, int outEfl>
  test_driver_float<10,-2, 1, 20, 3, 9>(max_error_arcsine_float, allowed_error);
  test_driver_float< 9,-3, 1, 20, 3, 9>(max_error_arcsine_float, allowed_error);
  test_driver_float<12, 3, 1, 24, 3, 9>(max_error_arcsine_float, allowed_error);
  test_driver_float< 5,-1, 1, 25, 3, 9>(max_error_arcsine_float, allowed_error);
  test_driver_float< 3,-1, 1, 25, 3, 9>(max_error_arcsine_float, allowed_error);
  test_driver_float< 8,-2, 1, 25, 3, 9>(max_error_arcsine_float, allowed_error);

  cout << "=============================================================================" << endl;
  cout << "  Testbench finished. Maximum errors observed across all bit-width variations:" << endl;
  cout << "    max_error_arcsine_fixed       = " << max_error_arcsine_fixed   << endl;
  cout << "    max_error_arcsine_float       = " << max_error_arcsine_float   << endl;

  // If error limits on any tested datatype have been crossed, the test has failed
  bool test_fail = max_error_arcsine_fixed > allowed_error || max_error_arcsine_float > allowed_error;
