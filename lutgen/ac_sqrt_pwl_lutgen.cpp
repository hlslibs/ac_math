...
int main()
{
  ...
  for (int i = 0; i < nsegments; i++) {
    ...
    double max_diff = (pwl_new(x_mid, m, c, prop_constant, x_min, nsegments) - sqrt(x_mid));
  ...
  if (x_min == 0.5 && x_max == 1) {
    ...
    // root2 value in the ac_sqrt_pwl header.
    ...
    // (ac_fixed<2*n_frac_bits, 1, false, q_mode_temp>, where q_mode_temp is set to AC_TRN by default)
    // If the user changes the precision/rounding from the default in the pwl header, they must change
    // it here too.
    const double rhl_1 = o_ac_f_trn((c_fixed[0]*root2).to_double(), 2*nfrac_bits - 1).to_double();

    while (rhl_1 < lhl_1) {
      // Keep lowering the end point of the last segment by subtracting a quantum value from the slope, until
      // the right hand limit is greater than/equal to the left hand limit.
  ...
  if (pc_is_po2) {
    ...
    outfile << "const int sc_input_frac_bits = AC_MAX(1, AC_MIN(n_frac_bits, W - " << log2_pc << "));" << endl;
  } else {
