 ...
 *************************************************************************/
// Usage:
//   g++ -I$MGC_HOME/shared/include ac_reciprocal_pwl_ha_lutgen.cpp -o ac_reciprocal_pwl_ha_lutgen
//   ./ac_reciprocal_pwl_ha_lutgen
// results in a text file ac_reciprocal_pwl_ha_lut_values.txt which can be pasted into
// a locally modified version of ac_reciprocal_pwl_ha.h.
...
int main()
{
  ...
  // as this .cpp file
  const char filename[] = "ac_reciprocal_pwl_ha_lut_values.txt";

  double x[upper_it + 1], x_sc[upper_it + 1], y[upper_it + 1], m[upper_it], c[upper_it];
  ac_fixed<nfrac_bits_u + 2, 2, true> m_fixed[upper_it], c_fixed[upper_it];
  ...
  for (unsigned nsegments = lower_it; nsegments <= upper_it;) {
    ...
    for (unsigned i = 0; i < nsegments; i++) {
      ...
      double max_diff = pwl_new(x_mid, m, c, prop_constant, x_min, nsegments) - 1/x_mid;
    ...
    for (double input_tb = x_min; input_tb < x_max; input_tb += increment) {
      const double expected = 1/input_tb;
  ...
  outfile << "const ac_fixed<" << x_max_int_bits << " + n_frac_bits, " << x_max_int_bits << ", " << is_neg_x_max_s << "> x_max_lut = " << o_ac_f(x_max, nfrac_bits_final) << ";" << endl;
  outfile << "const ac_fixed<" << sc_int_bits << " + n_frac_bits, " << sc_int_bits << ", false> prop_constant_lut = " << o_ac_f(prop_constant, nfrac_bits_final) << "; // Scaling constant" << endl;
  outfile.close();
