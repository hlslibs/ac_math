...
namespace ac_math
{
  ...
  void ac_sqrt_pwl (const ac_fixed < input_width, input_int, false, q_mode, o_mode> input, ac_fixed <output_width, output_int, false, q_mode_out, o_mode_out> &output)
  {
    ...
    ac_fixed <output_width, output_int, false, q_mode_out, o_mode_out> output_temp;
    ...
    // prop_constant is used to scale the input
    ...
    m1 = (normalized_exp_temp1 % 2 == 0) ? normalized_output : normalized_output_temp;
...
//-------------------------------------------------------------------------------------
  ...
  {
    const unsigned I = (input_int > 0) ? input_int : 1; // handles the condition for I<0
    const unsigned W = (input_width > input_int) ? input_width : input_width + input_int; // handles condition for W>I and I>0(handled by above statement).
    ...
    ac_fixed <4*W, 2*I, false, q_mode_in, o_mode_in> sqr_real = temp_real >> 1; // calculating square of the output's real part
    ac_fixed <4*W, 2*I, false, q_mode_in, o_mode_in> sqr_imag = temp_imag >> 1; // calculating square of the output's imaginary part
    ac_fixed <8*W, 2*I, false, q_mode_in, o_mode_in> x;
    ac_fixed <8*W, 2*I, false, q_mode_in, o_mode_in> y;
    ac_sqrt_pwl <q_mode_temp> (sqr_real, x); // calculating output's real part
    ac_sqrt_pwl <q_mode_temp> (sqr_imag, y); // calculating output's imaginary part
